...
def lambda_handler(event, context):
    ...
    try:
        data = body['data']
        """
        not use pre-signed url 2023. 1. 29
        just offer cloudfront cdn url
        """
        ...
        s3_signed_url = "http://{}/{}".format(CLOUDFRONT_DOMAIN, data['s3_key']) 
        logging.info("contents url: {}".format(s3_signed_url))
